...
pbxProject.prototype.updateBuildPropertyBy = function(prop, pred) {
    var config = this.pbxXCBuildConfigurationSection();
    propSomething(config, prop, pred);
}
...
function propSomething(obj, prop, pred) {
    for (var p in obj) {
        if (obj.hasOwnProperty(p)) {
            if (typeof obj[p] == 'object' && !Array.isArray(obj[p])) {
                propSomething(obj[p], prop, pred);
            } else if (p == prop) {
                obj[p] = pred(obj[p]);
            }
        }
